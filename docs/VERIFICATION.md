...
## 13. 12단계 — 인스턴스축(N) 규모 실측 (2026-07-17)

로드맵 12단계. 10단계는 시간축(dt)만 늘렸다(365dt×6inst). 이번엔 **인스턴스축(N)**을
늘려, 12단계 표의 외삽("소파일→추출→컴퓨트 순으로 무너질 것")을 실측으로 검증했다.

**설계 — 총량 고정, 축만 회전**: 10단계가 2,190파일·54.5M행이었으므로 N축을
**7dt×300inst=2,100파일·52.2M행**으로 맞췄다. 총량이 같으면 측정 차이가 전부 "축의
모양"(per-dt 파일 수 6→300)으로 귀속된다. 합성은 `scale_synthesize --instances N`
(신규): 닫힌 dt의 실제 인스턴스를 순환 복제 + instance_id 리매핑 + query_id `~i{j}`
suffix(카디널리티 자연 증가) + id 오프셋. 격리 프리픽스 `scale_n/`, 악화 주입 없음.

### 13-1. 실측 수치 (N=100 · N=300 · 10단계 dt축 대조)

| 지표 | dt축(365dt×6) 10단계 | N=100(7dt) | N=300(7dt) |
|---|---|---|---|
| 파일 수 · 총 행수 | 2,190 · 54.5M | 700 · 17.7M | 2,100 · 52.2M |
| 파일 평균 크기 | 177KB | 180KB | **177KB** (동일 프로파일) |
| 전체 글롭+count | 3,471ms | 3,954ms | 1,098ms* |
| per-dt 글롭(게이트 단위) | 8~22ms(6파일) | 154ms(100파일) | 33ms*(300파일) |
| **fct full-refresh** | **407.62s** | 68.08s | **769.21s** |
| **fct 증분(1dt 추가)** | 4s | 2.98s | **8.03s** |
| mart 재빌드 | 0.31s | 0.22s | 0.12s |
| 고유 query_id(1dt) | — | 9,039 | 26,700 (N 비례) |

\* ms 단위 글롭 수치는 직전 쿼리의 메타데이터 캐시 온도에 따라 흔들린다(N=300 측정은
②가 ③을 데워줌). 순서 있는 신호는 초 단위 항목들이다 — 정직 표기.

### 13-2. 판정 — 외삽이 틀렸다. 급소는 소파일이 아니라 full-refresh다

- **일상 경로(증분)는 N축에서 건재하다**: 새 dt 하나 = 300파일·7.5M행을 워터마크
  프루닝으로 8.03초에 처리(N=100의 2.98s에서 N에 대략 선형). **N=300에서도 daily
  파이프라인은 안 무너진다** — 12단계 노브(병렬도·컴팩션)를 만지기 전에도.
- **진짜 급소는 full-refresh(backfill·과거 정정)**: 같은 52~54M행인데 dt축 407.62s vs
  **N축 769.21s — 1.9배**. 축의 모양만 바꿨는데 전체 재빌드가 두 배 아프다. 가설:
  fct의 first/last 창 집계가 dt 파티션 단위로 도는데, dt축은 dt당 149k행(가벼운 창
  365개)인 반면 N축은 dt당 **7.46M행**(무거운 창 7개) — 창 하나의 정렬·스필 압력이
  50배 커진다. 원인 규명은 EXPLAIN 프로파일링이 필요해 가설로 남긴다(정직).
- **소파일은 예상보다 덜 아팠다**: 평균 177KB(128MB 타깃의 1/741)로 프로파일은
  10단계와 동일하지만, 전체 글롭·리스팅은 여전히 초 단위 이하. 소파일의 고통은
  "지금 아픈 것"이 아니라 파일 수가 훨씬 더 커질 때의 누적 위험으로 남는다.
- **추출(offload의 원천 PG 조회)은 이 합성으로 측정 불가** — 원천 PG에 N인스턴스가
  실재하지 않는다. N축 추출 병렬화의 실측은 실원천이 커질 때의 몫(정직 표기).

### 13-3. 격리·정리

생성 2,100(+증분 검증용 300) 오브젝트 전부 `scale_n/` 프리픽스에만. 측정 후
...
### 14-5. D1 편입 — wait_event_snapshot 추출 (2026-07-18 추가)

DBTower가 V25 + 수집 잡(5분 주기·7일 보존)을 낳아(그쪽 VERIFICATION 100절 이후 아크)
레지스트리 `available=True`로 편입했다. 예약 시절의 단일 `value` 대신 실형
(`wait_count`·`total_ms`)으로 스키마를 정정 — 계약은 실물이 정한다.

```
원천 첫 수집: 6인스턴스 134행 (MSSQL 37 · Oracle 50 · Mongo 6 · PG 3 — 기종별 실형)
offload dt=2026-07-18: 134행, 멱등 2회 불변
게이트: reconciliation OK · completeness SKIP · schema_drift OK
        freshness FAIL (열린 창 — 하루가 안 닫혔으니 차단이 정답. fail-closed 정상 동작)
```

freshness는 이 테이블 프로필에서 켜져 있다(5분 주기 연속 수집이라 닫힌 dt에선 경계
근접이 성립) — 열린 오늘 dt로 게이트를 돌린 위 FAIL은 결함이 아니라 계약의 증명이다.
pytest 57 회귀 0.

## 15. 13단계 — 용량 예측 (C1~C5, 2026-07-18)

DBTower V26(`size_snapshot` — 6시간 주기 tableStats 영속·7일 보존, 그쪽 102절)이 원료를
공급하기 시작해 13단계를 해제했다. 실측:

```
공급 첫 사이클: 6인스턴스 43오브젝트 (self-PG 185MB · PG 70MB · Mongo 5.5MB …)
레지스트리 편입: offload 43행 · 멱등 2회 불변 · 게이트 정합/드리프트 OK
  (completeness·freshness SKIP — 격리 인스턴스·6시간 저빈도 주기, 사유 명시)
dbt: stg_size_snapshot → fct_size_daily → mart_capacity_forecast + capacity_thresholds seed
산식 고정(unit test): 선형 +10MB/일·임계 2000MB → slope=10,000,000.0 · 잔여 81일 · d90 PASS
                      역성장 → days_to_threshold NULL · stable_or_shrinking PASS
실데이터: 전 인스턴스 learning=True (이력 1dt — 14일 게이트 미달. 지어내지 않는 초기 상태)
publish: 6마트 단일 트랜잭션 발행 (fct_size_daily 43행 · mart_capacity_forecast 6행)
CI: 픽스처에 size 소스 추가 — 로컬 전체 재현 dbt build PASS=44 (unit test 9)
```

**판정 컬럼까지만 계산하고 발화하지 않는다**(13단계 "발화 주체" 계약) — 소비는
Metabase(pull)·DBTower reverse ETL(push)의 몫. 잔여: C6 용량 대시보드는 learning이
풀리는 이력(14일)이 쌓이면 — 부트스트랩 스크립트 패턴으로 추가. volume_*(임계 원천
...
## 19. 19단계 — 상관 일반화·지연 축·지평 경계 (2026-07-18)

18단계 상관(플랜 뒤집힘 한 축·config 전용)을 넓혔다. 소스를 통합 스트림으로 일반화하고,
플랜이 안 뒤집혀도 켜지는 지연 축을 더했다. change_review는 짊어지지 않고 자리만 열었다.

```
int_change_events(view): 변경을 (instance_id, change_dt, change_source, change_key,
  change_kind, old/new_value) 한 형태로. config만 지금, schema(change_review) union all 자리
  주석으로 열어둠(주석 안 ref()는 dbt가 의존 파싱 → 일부러 미사용).
mart_config_impact: int_change_events 위에서 재계산 + 변경 전후 평균 지연(fct_query_daily).
  correlation 우선순위 followed_by_regression > plan_flip > latency_rise > no_signal.
mart_index_verdict 헤더 + ROADMAP 17단계: 지평 경계 명문화(DBTower 라이브 vs 90일 확정).
unit test 2종(regression follows·latency rise)·accepted_values(4값). dbt build PASS=111·pytest 57.
```

라이브 실측(dev 창고):

```
mart_config_impact 4행 — change_source=config, before_latency_ms 실측(inst2 3.1ms·inst4 35.04ms).
  after_latency_ms NULL(변경이 당일 07-18이라 후 N일 창이 미래 → 비었음). correlation=no_signal.
  → 정직: 후 N일이 지나면 지연 축이 켜진다(시간 해제). 플랜 축은 플랜 이력 축적 시.
CI 픽스처: 변경(01-01)↔뒤집힘(01-02) 겹침으로 followed_by_plan_flip 산출 유지.
MCP lakehouse_query 도구 설명에 mart_config_change·mart_config_impact 편입(stage 18 누락분).
Metabase "설정 드리프트" 대시보드 영향 카드 갱신(change_source·지연 전후·비율).
```
...
## 21. 21단계 — 가용성 SLO (2026-07-19)

DBTower가 35일 뒤 지우는 up 여부(health_sample)를 장기 가용성 SLO로. 전수 대조에서 두 잣대
(DBTower가 지움 + 장기 판정)를 다 통과하는 유일한 미착수 원천이자, Prometheus에도 없던 빈 자리.

```
health_sample 레지스트리(sampled_at 워터마크, completeness 끔). run_offload_aux 자동 편입.
fct_uptime_daily: 1분 샘플 → 일별 uptime_pct·avg/p95 ping(up 샘플만 — 다운 ping은 타임아웃).
mart_uptime_slo: 최근 30일 uptime vs 목표(seed uptime_slo_targets/var) → SRE 에러버짓·
  slo_status(meets/at_risk/breach). dim_instance 조인(기종).
mart_weekly_ops_report: this_week_uptime_pct 편입.
unit test 2종(uptime 75% 산식·breach/meets)·accepted_values 3값. dbt build PASS=129·pytest 57.
```

라이브 실측(dev 창고, 4일 창 — 상관 마트와 달리 즉시 의미 있게 켜짐):

```
health_sample 오프로드: 07-15~18(07-14는 원천에 그날 UTC 샘플 0 — 정직).
mart_uptime_slo 7행:
  local-mssql  63.45% (worst 41.19%, ping 2132ms) → breach (목표 99.9)
  mssql-pitr   64.28% (ping 6897ms)               → breach
  local-mysql  98.86% (worst 96.83%, ping 9ms)    → breach (목표 99.5 근소 미달)
  local-postgres 99.90% · local-mongo 99.91% · local-oracle 99.91% → meets
  dbtower-self 100.0% → meets
기종 라벨과 함께 "어느 기종이 몇 % 떴나"가 한 줄로. MSSQL 두 대의 가용성 문제가 즉시 드러남.
주간 보고 uptime 칸, Metabase "가용성 SLO" 대시보드 실화면. MCP 도구 설명에 uptime 마트 편입.
```

정직한 표기: 가용성은 연속 측정이라 상관 마트(후행 관측 대기)와 달리 바로 채워진다. down
샘플의 ping은 up 샘플로만 평균해 지연 통계 오염을 막았다. 이로써 창고의 판정이 6종
(용량·플랜·백업·인덱스·설정 상관·가용성)이 되어 마지막 빈 자리가 채워졌다.
